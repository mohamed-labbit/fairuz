...
public:
  enum class GrowthStrategy : std::int32_t { LINEAR, EXPONENTIAL };
...
public:
  ...
  template <typename T> T *allocateArray(size_t const count) { return static_cast<T *>(allocate(count * sizeof(T))); }

  template <typename T> void deallocateArray(T *ptr, size_t const count) { deallocate(static_cast<void *>(ptr), count * sizeof(T)); }

  template <typename T, typename... Args> MY_NODISCARD T *allocateObject(Args &&...args) {
    static_assert(std::is_constructible_v<T, Args...>, "T must be constructible with Args...");
    return ::new (allocate(sizeof(T))) T(std::forward<Args>(args)...);
  }

  template <typename T> void deallocateObject(T *obj) { deallocate(static_cast<void *>(obj), sizeof(T)); }
