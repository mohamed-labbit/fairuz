...
public:
  template <typename CodeEnum> explicit Error(CodeEnum code) : m_code(static_cast<uint16_t>(code)) {}

  template <typename CodeEnum> Error() : m_code(CodeEnum::UNKNOWN) {}
...
template <typename T, typename E = Error> class [[nodiscard]] ErrorOr {
public:
  ErrorOr(T val) : is_value_(true) { ::new (static_cast<void *>(&storage_)) T(static_cast<T &&>(val)); }

  ErrorOr(E err) : is_value_(false) { ::new (static_cast<void *>(&storage_)) E(static_cast<E &&>(err)); }

  static ErrorOr fromValue(T v) { return ErrorOr(static_cast<T &&>(v)); }
  static ErrorOr fromError(E e) { return ErrorOr(static_cast<E &&>(e)); }

  ErrorOr(ErrorOr const &other) : is_value_(other.is_value_) {
    if (is_value_)
