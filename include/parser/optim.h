...
class ASTOptimizer
{
   private:
    ...
    std::optional<double> evaluateConstant(const Expr* expr)
    {
        if (!expr)
        {
            return std::nullopt;
        ...
        if (expr->kind_ == Expr::Kind::LITERAL)
        {
            auto* lit = static_cast<const LiteralExpr*>(expr);
            if (lit->litType_ == LiteralExpr::Type::NUMBER)
            {
                try
                {
        ...
        else if (expr->kind_ == Expr::Kind::BINARY)
        {
        ...
        else if (expr->kind_ == Expr::Kind::UNARY)
        {
   ...
   public:
    ...
    ExprPtr optimizeConstantFolding(ExprPtr expr)
    {
        if (!expr)
        {
            return expr;
        ...
        if (expr->kind_ == Expr::Kind::BINARY)
        {
            ...
            if (auto val = evaluateConstant(expr.get()))
            {
                ...
                return std::make_unique<LiteralExpr>(LiteralExpr::Type::NUMBER, utf8::utf8to16(std::to_string(*val)));
            }
            ...
            // x + 0 = x, x - 0 = x
            if ((bin->op_ == u"+" || bin->op_ == u"-") && right->kind_ == Expr::Kind::LITERAL)
            {
                auto* lit = static_cast<LiteralExpr*>(right);
                if (lit->value_ == u"0")
                {
            ...
            // x * 0 = 0
            if (bin->op_ == u"*" && right->kind_ == Expr::Kind::LITERAL)
            {
                auto* lit = static_cast<LiteralExpr*>(right);
                if (lit->value_ == u"0")
                {
                    stats_.strengthReductions_++;
                    return std::make_unique<LiteralExpr>(LiteralExpr::Type::NUMBER, u"0");
                }
            }
            ...
            if (bin->op_ == u"*" && right->kind_ == Expr::Kind::LITERAL)
            {
                ...
                if (lit->value_ == u"2")
                {
                    ...
                    auto leftClone = std::make_unique<NameExpr>(static_cast<NameExpr*>(left)->name_);
                    return std::make_unique<BinaryExpr>(std::move(bin->left_), u"+", std::move(leftClone));
                }
            }

            // x - x = 0
            if (bin->op_ == u"-" && left->kind_ == Expr::Kind::NAME && right->kind_ == Expr::Kind::NAME)
            {
                auto* lname = static_cast<NameExpr*>(left);
                auto* rname = static_cast<NameExpr*>(right);
        ...
        else if (expr->kind_ == Expr::Kind::UNARY)
        {
            ...
            // Double negation: --x = x
            if (un->op_ == u"-" && un->operand_->kind_ == Expr::Kind::UNARY)
            {
                auto* innerUn = static_cast<UnaryExpr*>(un->operand_.get());
                if (innerUn->op_ == u"-")
                {
                    stats_.strengthReductions_++;
        ...
        else if (expr->kind_ == Expr::Kind::CALL)
        {
        ...
        else if (expr->kind_ == Expr::Kind::LIST)
        {
            auto* list = static_cast<ListExpr*>(expr.get());
            for (auto& elem : list->elements)
            {
                elem = optimizeConstantFolding(std::move(elem));
            }
        }
        else if (expr->kind_ == Expr::Kind::TERNARY)
        {
            ...
            if (tern->condition_->kind_ == Expr::Kind::LITERAL)
            {
                auto* lit = static_cast<LiteralExpr*>(tern->condition_.get());
                if (lit->litType_ == LiteralExpr::Type::BOOLEAN)
                {
                    stats_.constantFolds_++;
    ...
    StmtPtr eliminateDeadCode(StmtPtr stmt)
    {
        if (!stmt)
        {
            return stmt;
        ...
        if (stmt->kind_ == Stmt::Kind::IF)
        {
            auto* ifStmt = static_cast<IfStmt*>(stmt.get());

            // Constant condition elimination
            if (ifStmt->condition->kind_ == Expr::Kind::LITERAL)
            {
                auto* lit = static_cast<LiteralExpr*>(ifStmt->condition.get());
                if (lit->litType_ == LiteralExpr::Type::BOOLEAN)
                {
                    ...
                    if (lit->value_ == u"true")
                    {
                        ...
                        return std::make_unique<BlockStmt>(std::move(ifStmt->thenBlock));
                    }
                    else
                    {
                        ...
                        {
                            return std::make_unique<BlockStmt>(std::move(ifStmt->elseBlock));
                        }
            ...
            std::vector<StmtPtr> newThen, newElse;
        ...
        else if (stmt->kind_ == Stmt::Kind::WHILE)
        {
            auto* whileStmt = static_cast<WhileStmt*>(stmt.get());

            // Infinite loop with false condition
            if (whileStmt->condition->kind_ == Expr::Kind::LITERAL)
            {
                auto* lit = static_cast<LiteralExpr*>(whileStmt->condition.get());
                if (lit->litType_ == LiteralExpr::Type::BOOLEAN && lit->value_ == u"false")
                {
                    stats_.deadCodeEliminations_++;
        ...
        else if (stmt->kind_ == Stmt::Kind::FOR)
        {
        ...
        else if (stmt->kind_ == Stmt::Kind::FUNCTION_DEF)
        {
            auto* funcDef = static_cast<FunctionDef*>(stmt.get());
    ...
    class CSEPass
    {
       private:
        ...
        std::u16string exprToString(const Expr* expr)
        {
            if (!expr)
            {
                return u"";
       ...
       public:
        ...
        std::optional<std::u16string> findCSE(const Expr* expr)
        {
        ...
        void recordExpr(const Expr* expr, const std::u16string& var)
        {
            std::u16string exprStr = exprToString(expr);
            if (!exprStr.empty())
            {
    ...
    bool isLoopInvariant(const Expr* expr, const std::unordered_set<std::u16string>& loopVars)
    {
        if (!expr)
        {
            return true;
        ...
        else if (expr->kind_ == Expr::Kind::LITERAL)
        {
            return true;
        }
   ...
   public:
    ...
    std::vector<StmtPtr> optimize(std::vector<StmtPtr> statements, int level = 2)
    {
