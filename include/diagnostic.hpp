...
enum class Severity : uint8_t { NOTE, FATAL, ERROR, WARNING };
...
static void internalError(errc::general::Code err_code) { engine.emit(errorMessageFor(static_cast<uint16_t>(err_code))); }

static void runtimeError(errc::runtime::Code err_code) { engine.emit(errorMessageFor(static_cast<uint16_t>(err_code))); }
