...
template <typename T> void Array<T>::ensure_push_capacity() {
  if (Size_ < Cap_)
    return;
...
template <typename T> Array<T>::Array(uint32_t capacity, T fill_v) {
...
template <typename T> Array<T>::Array(Array const &other) : Cap_(other.Cap_) {
  if (Cap_ == 0)
    return;
...
template <typename T> Array<T>::Array(Array &&other) noexcept : arr(other.arr), Size_(other.Size_), Cap_(other.Cap_) {
  other.arr = nullptr;
  other.Size_ = 0;
  other.Cap_ = 0;
}
...
template <typename T> void Array<T>::push(T const &val) {
  ensure_push_capacity();
  if constexpr (trivial_copy)
    arr[Size_] = val;
  else
...
template <typename T> void Array<T>::reserve(uint32_t const s) {
  if (s <= Cap_)
    return;
...
template <typename T> void Array<T>::resize(uint32_t const s) {
  if (s < Size_) {
...
template <typename T> void Array<T>::erase(uint32_t const at) {
  if (at >= Size_)
...
template <typename T> T *Array<T>::erase(T const *p) {
...
template <typename T, typename... Args> static Array<T> makeArray(Args &&...args) {
