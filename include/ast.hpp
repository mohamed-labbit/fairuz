...
public:
  enum class Kind : int { INVALID, BINARY, UNARY, LITERAL, NAME, CALL, ASSIGNMENT, LIST, INDEX };

protected:
  Kind Kind_{Kind::INVALID};

public:
  Expr() : Kind_(Kind::INVALID) {}
...
public:
  ...
  BinaryExpr(Expr *l, Expr *r, BinaryOp op) : Left_(l), Right_(r), Operator_(op) { Kind_ = Kind::BINARY; }
...
public:
  ...
  UnaryExpr(Expr *operand, UnaryOp op) : Operand_(operand), Operator_(op) {
    Kind_ = Kind::UNARY;
...
public:
  enum class Type { INTEGER, FLOAT, HEX, OCTAL, BINARY, STRING, BOOLEAN, NIL };
...
public:
  explicit LiteralExpr() : Type_(Type::NIL) { Kind_ = Kind::LITERAL; }

  LiteralExpr(int64_t value, Type type) : Type_(type), IntValue_(value) { Kind_ = Kind::LITERAL; }
  LiteralExpr(double value, Type type) : Type_(type), FloatValue_(value) { Kind_ = Kind::LITERAL; }

  explicit LiteralExpr(bool value) : Type_(Type::BOOLEAN), BoolValue_(value) { Kind_ = Kind::LITERAL; }
  explicit LiteralExpr(StringRef str) : Type_(Type::STRING), StrValue_(std::move(str)) { Kind_ = Kind::LITERAL; }
...
public:
  ...
  explicit NameExpr(StringRef const &s) : Value_(s) {
    Kind_ = Kind::NAME;
...
public:
  ...
  explicit ListExpr(Array<Expr *> elements) : Elements_(std::move(elements)) { Kind_ = Kind::LIST; }
...
public:
  ...
  explicit CallExpr(Expr *c, ListExpr *a = nullptr, CallLocation loc = CallLocation::GLOBAL) : Callee_(c), Args_(a), CallLocation_(loc) {
    Kind_ = Kind::CALL;
  }
...
public:
  ...
  AssignmentExpr(Expr *target, Expr *value, bool decl = false) : Target_(target), Value_(value), isDecl_(decl) { Kind_ = Kind::ASSIGNMENT; }
...
public:
  IndexExpr(Expr *obj, Expr *idx) : Object_(obj), Index_(idx) { Kind_ = Kind::INDEX; }
...
public:
  enum class Kind : uint8_t { INVALID, EXPR, ASSIGNMENT, IF, WHILE, FOR, FUNC, RETURN, BLOCK };
...
public:
  ...
  explicit BlockStmt(Array<Stmt *> stmts) : Statements_(stmts) { Kind_ = Kind::BLOCK; }
...
public:
  ...
  explicit ExprStmt(Expr *expr) : Expr_(expr) { Kind_ = Kind::EXPR; }
...
public:
  ...
  AssignmentStmt(Expr *target, Expr *value, bool decl = false) : Target_(target), Value_(value), isDecl_(decl) { Kind_ = Kind::ASSIGNMENT; }
...
public:
  ...
  IfStmt(Expr *condition, BlockStmt *then_block, BlockStmt *else_block) : Condition_(condition), ThenBlock_(then_block), ElseBlock_(else_block) {
    Kind_ = Kind::IF;
  }
...
public:
  ...
  WhileStmt(Expr *condition, BlockStmt *block) : Condition_(condition), Block_(block) { Kind_ = Kind::WHILE; }
...
public:
  ...
  ForStmt(NameExpr *target, Expr *iter, BlockStmt *block) : Target_(target), Iter_(iter), Block_(block) { Kind_ = Kind::FOR; }
...
public:
  ...
  FunctionDef(NameExpr *name, ListExpr *params, BlockStmt *body) : Name_(name), Params_(params), Body_(body) {
    Kind_ = Kind::FUNC;
...
public:
  ...
  explicit ReturnStmt(Expr *value) : Value_(value) { Kind_ = Kind::RETURN; }
...
static ErrStmt *makeErrStmt(uint32_t line, uint32_t col) { return getAllocator().allocateObject<ErrStmt>(line, col); }
static ErrExpr *makeErrExpr(uint32_t line, uint32_t col) { return getAllocator().allocateObject<ErrExpr>(line, col); }
static BinaryExpr *makeBinary(Expr *l, Expr *r, BinaryOp const op) { return getAllocator().allocateObject<BinaryExpr>(l, r, op); }
static UnaryExpr *makeUnary(Expr *operand, UnaryOp const op) { return getAllocator().allocateObject<UnaryExpr>(operand, op); }
static LiteralExpr *makeLiteralNil() { return getAllocator().allocateObject<LiteralExpr>(); }
...
static LiteralExpr *makeLiteralInt(int64_t value) { return getAllocator().allocateObject<LiteralExpr>(value, LiteralExpr::Type::INTEGER); }
static LiteralExpr *makeLiteralFloat(double value) { return getAllocator().allocateObject<LiteralExpr>(value, LiteralExpr::Type::FLOAT); }
static LiteralExpr *makeLiteralString(StringRef value) { return getAllocator().allocateObject<LiteralExpr>(value); }
static LiteralExpr *makeLiteralBool(bool value) { return getAllocator().allocateObject<LiteralExpr>(value); }
static NameExpr *makeName(StringRef const str) { return getAllocator().allocateObject<NameExpr>(str); }
static ListExpr *makeList(Array<Expr *> elements) { return getAllocator().allocateObject<ListExpr>(elements); }
static CallExpr *makeCall(Expr *callee, ListExpr *args) { return getAllocator().allocateObject<CallExpr>(callee, args); }
...
static IndexExpr *makeIndex(Expr *obj, Expr *idx) { return getAllocator().allocateObject<IndexExpr>(obj, idx); }
static BlockStmt *makeBlock(Array<Stmt *> stmts) { return getAllocator().allocateObject<BlockStmt>(stmts); }
static ExprStmt *makeExprStmt(Expr *expr) { return getAllocator().allocateObject<ExprStmt>(expr); }
...
static WhileStmt *makeWhile(Expr *condition, BlockStmt *block) { return getAllocator().allocateObject<WhileStmt>(condition, block); }
static ForStmt *makeFor(NameExpr *target, Expr *iter, BlockStmt *block) { return getAllocator().allocateObject<ForStmt>(target, iter, block); }
...
static ReturnStmt *makeReturn(Expr *value = nullptr) { return getAllocator().allocateObject<ReturnStmt>(value); }
