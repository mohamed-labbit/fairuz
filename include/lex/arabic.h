...
static inline bool is_ascii_letter(char32_t c)
{
...
static inline bool is_arabic_letter(char32_t c)
{
    ...
    return (static_cast<int>(c) >= 0x0600 && static_cast<int>(c) <= 0x06FF) ||  // Arabic
           ...
           (static_cast<int>(c) >= 0xFB50 && static_cast<int>(c) <= 0xFDFF)
        ||  // Arabic Presentation Forms-A
           (static_cast<int>(c) >= 0xFE70
            && static_cast<int>(c) <= 0xFEFF);  // Arabic Presentation Forms-B
}
...
static inline bool is_arabic_combining_mark(char32_t c)
{
    ...
    return (static_cast<int>(c) >= 0x064B && static_cast<int>(c) <= 0x065F)
        || (static_cast<int>(c) == 0x0670);
}

static inline bool is_ident_start(char32_t c)
{
    return c == U'_' || is_ascii_letter(c) || is_arabic_letter(c);
}

static inline bool is_ident_continue(char32_t c)
{
    return is_ident_start(c) || is_ascii_digit(c) || is_arabic_digit(c)
        || is_arabic_combining_mark(c);
}