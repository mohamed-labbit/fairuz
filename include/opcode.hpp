...
inline uint32_t make_ABSC(OpCode op, uint8_t A, uint8_t B, int8_t sC) {
  return (static_cast<uint32_t>(op) << 24) | (static_cast<uint32_t>(A) << 16) | (static_cast<uint32_t>(B) << 8) |
         static_cast<uint32_t>(static_cast<uint8_t>(sC + 128));
}

inline int8_t instr_sC(uint32_t i) { return static_cast<int8_t>(i & 0xFF) - 128; }

enum class InstrFormat : uint8_t { ABC, ABx, AsBx, A, NONE };
...
template <typename... Args> static Chunk *makeChunk(Args &&...args) { return getAllocator().allocateObject<Chunk>(std::forward<Args>(args)...); }
