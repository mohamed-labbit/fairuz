...
allocator::ArenaAllocator __allocator;

static uint32_t node_line(ASTNode const* n)
{
    return n ? n->getLine() : 0;
}
...
Chunk* Compiler::compile(std::vector<ast::Stmt*> const& stmts)
{
    ...
    if (!isDead_)
        emit(make_ABC(static_cast<uint8_t>(OpCode::RETURN_NIL), 0, 0, 0), node_line(stmts.back()));
...
void Compiler::compileStmt(Stmt const* s)
{
    ...
    default:
        error("invalid statement node", node_line(s));
        break;
    }
}
...
void Compiler::compileReturn(ReturnStmt const* s)
{
    ...
    if (val->getKind() == Expr::Kind::CALL) {
        Reg mark = Current_->nextReg;
        compileCall(static_cast<CallExpr const*>(val), std::nullopt, /*tail_pos=*/true);
        freeRegsTo(mark);
        markDead();
        return;
    }
...
Reg Compiler::compileExpr(Expr const* e, std::optional<Reg> dst)
{
    if (!e)
        return errorReg();
    ...
    default:
        error("invalid expression node", node_line(e));
        return errorReg();
    }
}

Reg Compiler::compileLiteral(LiteralExpr const* e, std::optional<Reg> dst)
{
    uint32_t line = node_line(e);
    Reg d = ensureDst(dst);
...
Reg Compiler::compileName(NameExpr const* e, std::optional<Reg> dst)
{
    uint32_t line = node_line(e);
...
Reg Compiler::compileBinary(BinaryExpr const* e, std::optional<Reg> dst)
{
    uint32_t line = node_line(e);
    ...
    if (op == BinaryOp::OP_AND) {
        ...
        freeRegsTo(mark);
        Reg right = compileExpr(e->getRight(), d);

        if (right != d)
            emit(make_ABC(static_cast<uint8_t>(OpCode::MOVE), d, right, 0), line);
...
Reg Compiler::compileUnary(UnaryExpr const* e, std::optional<Reg> dst)
{
    uint32_t line = node_line(e);
...
Reg Compiler::compileAssignmentExpr(AssignmentExpr const* e, std::optional<Reg> dst)
{
    uint32_t line = node_line(e);
...
Reg Compiler::compileCall(CallExpr const* e, std::optional<Reg> dst, bool tail_pos)
{
    uint32_t line = node_line(e);
    ...
    Reg d = dst.value_or(func_reg); // reuse func_reg for result if no dst
    if (d != func_reg)
...
Reg Compiler::compileList(ListExpr const* e, std::optional<Reg> dst)
{
    uint32_t line = node_line(e);
    Reg d = ensureDst(dst);
...
Chunk* Compiler::compileFunction(StringRef const& name, std::vector<StringRef> const& params, Stmt const* body, uint32_t line)
{
    ...
    auto fn_chunk_owned = std::make_unique<Chunk>();
    fn_chunk_owned->name = name;
    fn_chunk_owned->arity = static_cast<int>(params.size());
    Chunk* fn_chunk = fn_chunk_owned.get();
    currentChunk()->functions.push_back(fn_chunk_owned.release());
    ...
    if (body->getKind() == Stmt::Kind::BLOCK) {
        // Don't double-wrap in a scope — the block IS the function body
        for (Stmt const* s : static_cast<BlockStmt const*>(body)->getStatements())
            compileStmt(s);
    } else
        compileStmt(body);
...
Reg Compiler::ensureDst(std::optional<Reg> dst)
{
    return dst.value_or(allocReg());
}
...
bool Compiler::isIntegerValue(double d, int64_t& out)
{
    if (!std::isfinite(d))
        return false;
