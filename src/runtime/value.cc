...
{
    return (Raw_ & NANBOX_QNAN) != NANBOX_QNAN || Raw_ == NANBOX_QNAN;
}

bool Value::isInteger() const
{
    return (Raw_ & ~UINT64_C(0xFFFFFFFFFFFF)) == TAG_INTEGER;
}
...
{
    return isObject() && asObject()->type == ObjType::FUNCTION;
}

bool Value::isClosure() const
{
    return isObject() && asObject()->type == ObjType::CLOSURE;
}

bool Value::isNative() const
{
    return isObject() && asObject()->type == ObjType::NATIVE;
}
