...
Fa_BinaryExpr* Fa_BinaryExpr::clone() const { return Fa_makeBinary(m_left->clone(), m_right->clone(), m_operator); }
...
Fa_UnaryExpr* Fa_UnaryExpr::clone() const { return Fa_makeUnary(m_operand->clone(), m_operator); }
...
Fa_LiteralExpr* Fa_LiteralExpr::clone() const
{
    switch (m_type) {
    case Type::INTEGER:
        return Fa_makeLiteralInt(int_value);
    case Type::FLOAT:
        return Fa_makeLiteralFloat(float_value);
    case Type::BOOLEAN:
        return Fa_makeLiteralBool(bool_value);
    case Type::STRING:
        return Fa_makeLiteralString(str_value);
    case Type::NIL:
...
Fa_ListExpr* Fa_ListExpr::clone() const { return Fa_makeList(m_elements); }
...
Fa_CallExpr* Fa_CallExpr::clone() const { return Fa_makeCall(m_callee->clone(), m_args->clone()); }
...
Fa_Expr* Fa_DictExpr::clone() const { return Fa_makeDict(content); }
...
Fa_BlockStmt* Fa_BlockStmt::clone() const { return Fa_makeBlock(m_statements); }
...
Fa_ExprStmt* Fa_ExprStmt::clone() const { return Fa_makeExprStmt(m_expr->clone()); }
...
{
    return Fa_makeAssignmentStmt(m_expr->get_target(), m_expr->get_value(), m_expr->is_declaration());
}
...
{
    return Fa_makeIf(m_condition->clone(), m_then_stmt->clone(), LIKELY(m_else_stmt == nullptr) ? nullptr : m_else_stmt->clone());
}
...
Fa_WhileStmt* Fa_WhileStmt::clone() const { return Fa_makeWhile(m_condition->clone(), m_body->clone()); }
...
{
    return get_allocator().allocate_object<Fa_ForStmt>(m_container->clone(), m_iter->clone(), m_body->clone());
}
...
{
    return Fa_makeFunction(m_name->clone(), m_params->clone(), m_body->clone());
}
