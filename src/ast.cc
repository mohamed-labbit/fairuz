...
bool IfStmt::equals(Stmt const *other) const {
  ...
  return Condition_->equals(block->getCondition()) && ThenBlock_->equals(block->getThenBlock()) && ElseBlock_->equals(block->getElseBlock());
}
...
bool ForStmt::equals(Stmt const *other) const {
  ...
  return Target_->equals(block->getTarget()) && Iter_->equals(block->getIter()) && Block_->equals(block->getBlock());
}

ForStmt *ForStmt::clone() const { return getAllocator().allocateObject<ForStmt>(Target_->clone(), Iter_->clone(), Block_->clone()); }
