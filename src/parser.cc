...
template<typename T>
static T* stamp_from_token(T* node, TokenPtr token)
{
...
void Fa_SemanticAnalyzer::report_issue(Issue::Severity sev, SemaCode code, Fa_StringRef msg, i32 line, Fa_StringRef const& sugg)
{
    m_issues.push({ sev, static_cast<u16>(code), msg, line, sugg });
}
...
void Fa_SemanticAnalyzer::analyze_expr(ExprPtr expr)
{
    ...
    case AST::Fa_Expr::Kind::NAME: {
        ...
        if (!m_current_scope->is_defined(name->get_value()))
            report_issue(Issue::Severity::ERROR, SemaCode::UNDEFINED_VARIABLE, "Undefined variable: " + name->get_value(), expr->get_line(), "Did you forget to initialize it?");
        else
            m_current_scope->mark_used(name->get_value(), expr->get_line());
    } break;
    ...
    case AST::Fa_Expr::Kind::CALL: {
        ...
        if (call->get_callee()->get_kind() == AST::Fa_Expr::Kind::NAME) {
            ...
            if (Fa_SymbolTable::Symbol* sym = m_current_scope->lookup(name->get_value())) {
                if (sym->symbol_type != Fa_SymbolTable::SymbolType::FUNCTION)
                    report_issue(Issue::Severity::ERROR, SemaCode::NOT_CALLABLE, "'" + name->get_value() + "' is not callable", expr->get_line());
            }
        }
        ...
        if (call->get_callee()->get_kind() == AST::Fa_Expr::Kind::NAME) {
            ...
            if (sym == nullptr)
                report_issue(Issue::Severity::ERROR, SemaCode::UNDEFINED_FUNCTION, "Undefined function: " + name->get_value(), expr->get_line());
...
void Fa_SemanticAnalyzer::analyze(Fa_Array<StmtPtr> const& stmts)
{
    ...
    for (Fa_SymbolTable::Symbol* sym : unused)
        report_issue(Issue::Severity::WARNING, SemaCode::UNUSED_VARIABLE, "Unused variable: " + sym->name, sym->definition_line, "Consider removing if not needed");
    ...
    for (Issue const& issue : m_issues) {
        ...
        diagnostic::report(sev, issue.line, 0, issue.code);
...
void Fa_SemanticAnalyzer::print_report() const
{
    ...
    for (Fa_SemanticAnalyzer::Issue const& issue : m_issues) {
        ...
        std::cout << "[" << sev_str << "] Line " << issue.line << ": " << issue.message << "\n";
...
Fa_ErrorOr<StmtPtr> Fa_Parser::parse_while_stmt()
{
    ...
    return stamp_from_token(Fa_makeWhile(condition.value(), static_cast<AST::Fa_BlockStmt*>(while_block.value())), start);
}
...
Fa_ErrorOr<StmtPtr> Fa_Parser::parse_indented_block()
{
    ...
    if (match(TokType::DEDENT))
        return stamp_from_token(Fa_makeBlock(stmts), start);
    ...
    auto* block = Fa_makeBlock(stmts);
    if (!stmts.empty())
        return stamp_from_node(block, stmts[0]);
...
Fa_ErrorOr<ExprPtr> Fa_Parser::parse_parameters_list()
{
    ...
    if (parameters.empty())
        return stamp_from_token(Fa_makeList(Fa_Array<ExprPtr> { }), current_token());
...
Fa_ErrorOr<ExprPtr> Fa_Parser::parse_assignment_expr()
{
    ...
    if (check(TokType::OP_ASSIGN) || /*augmented assign */ check(TokType::OP_PLUSEQ) || check(TokType::OP_MINUSEQ) 
        ...
        if (check(TokType::OP_PLUSEQ)|| check(TokType::OP_MINUSEQ) || check(TokType::OP_STAREQ) || check(TokType::OP_SLASHEQ)) {
            ...
            AST::Fa_BinaryOp bin_ops[] = { AST::Fa_BinaryOp::OP_ADD, AST::Fa_BinaryOp::OP_SUB, AST::Fa_BinaryOp::OP_MUL, AST::Fa_BinaryOp::OP_DIV };
            ...
            AST::Fa_BinaryExpr* bin = AST::Fa_makeBinary(c_lhs, rhs.value(), op);
            return Fa_ErrorOr<ExprPtr>::from_value(stamp_from_node(Fa_makeAssignmentExpr(target, bin, /*decl=*/false), target));
        }
        ...
        if (kind == AST::Fa_Expr::Kind::NAME) {
            ...
            return Fa_ErrorOr<ExprPtr>::from_value(
                stamp_from_node(Fa_makeAssignmentExpr(name_target, rhs.value(), decl=false), name_target)); // let sema detect decl vs assign
        }
        */

        return Fa_ErrorOr<ExprPtr>::from_value(stamp_from_node(Fa_makeAssignmentExpr(target, rhs.value(), /*decl=*/false), target));
    }
...
Fa_ErrorOr<ExprPtr> Fa_Parser::parse_logical_expr_precedence(unsigned int min_precedence)
{
    ...
    for (;;) {
        ...
        lhs.set_value(stamp_from_node(Fa_makeBinary(lhs.value(), rhs.value(), to_binary_op(op)), lhs.value()));
    }
...
Fa_ErrorOr<ExprPtr> Fa_Parser::parse_binary_expr_precedence(unsigned int min_precedence)
{
    ...
    for (;;) {
        ...
        lhs.set_value(Fa_makeBinary(lhs.value(), rhs.value(), to_binary_op(op)));
    }
...
Fa_ErrorOr<ExprPtr> Fa_Parser::parse_unary_expr()
{
    ...
    if (cur_tok->is_unary_op()) {
        ...
        return stamp_from_token(Fa_makeUnary(expr.value(), to_unary_op(op)), cur_tok);
    }
...
Fa_ErrorOr<ExprPtr> Fa_Parser::parse_postfix_expr()
{
    ...
    for (;;) {
        if (check(TokType::LPAREN)) {
            ...
            expr = stamp_from_node(Fa_makeCall(expr, stamp_from_token(Fa_makeList(std::move(args)), start)), expr);
            continue;
        }
        ...
        if (match(TokType::LBRACKET)) {
            ...
            expr = stamp_from_node(Fa_makeIndex(expr, index.value()), expr);
            continue;
        }
...
Fa_ErrorOr<ExprPtr> Fa_Parser::parse_primary_expr()
{
    ...
    if (cur_tok->is_numeric()) {
        ...
        if (tt == TokType::DECIMAL)
            return stamp_from_token(AST::Fa_makeLiteralFloat(v.to_double()), cur_tok);
    ...
    if (match(TokType::STRING))
        return stamp_from_token(AST::Fa_makeLiteralString(cur_tok->lexeme()), cur_tok);

    if (check(TokType::KW_TRUE) || check(TokType::KW_FALSE)) {
        advance();
        return stamp_from_token(AST::Fa_makeLiteralBool(cur_tok->lexeme() == "صحيح" ? true : false), cur_tok);
    }

    if (match(TokType::KW_NIL))
        return stamp_from_token(AST::Fa_makeLiteralNil(), cur_tok);

    if (match(TokType::IDENTIFIER))
        return stamp_from_token(AST::Fa_makeName(cur_tok->lexeme()), cur_tok);

    if (match(TokType::LPAREN)) {
        if (match(TokType::RPAREN))
            return stamp_from_token(Fa_makeList(Fa_Array<ExprPtr> { }), cur_tok);
...
Fa_ErrorOr<ExprPtr> Fa_Parser::parse_list_literal()
{
    ...
    if (elements.empty())
        return stamp_from_token(Fa_makeList(std::move(elements)), start);
    return stamp_from_node(Fa_makeList(std::move(elements)), first);
}
...
Fa_ErrorOr<ExprPtr> Fa_Parser::parse_logical_expr() { return parse_logical_expr_precedence(0); }
