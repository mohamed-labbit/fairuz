...
void DiagnosticEngine::report(Severity const sev, uint32_t const line,
    uint16_t const col, uint16_t err_code, std::string const& code)
{
...
// emit() is the eager path — used by components that need immediate
// feedback (e.g. the arena allocator hitting an invariant). It writes
// directly to stderr and does NOT accumulate, so it intentionally
// bypasses the deferred strategy. FATAL throws to unwind.
...
void DiagnosticEngine::prettyPrint() const
{
    ...
    // saturation notice — lets the user know they may have more errors
    // than shown, without pretending the list is complete
    if (isSaturated())
        std::cerr << Color::BOLD << Color::YELLOW << "warning" << Color::RESET
                  << ": " << ErrorCount_ << " errors reported, "
