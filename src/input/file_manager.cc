...
StringRef FileManager::readLine(SizeType const line_number)
{
    seekToLine(line_number);
    SizeType const LineLength = LineIndices_[line_number].LineLength;
...
std::vector<StringRef> FileManager::readLines(SizeType const start, SizeType const count)
{
    if (!LineIndexBuilt_)
        buildLineIndex();
    ...
    SizeType const end = std::min(start + count, LineIndices_.size());
...
CharType FileManager::peekChar(SizeType const CharOffset)
{
    pushPosition();
...
StringRef FileManager::peekRange(SizeType const start_offset, SizeType const length)
{
    pushPosition();
