...
#define DISPATCH_TABLE_INIT                                                                                                                          \
  &&H_LOAD_NIL, &&H_LOAD_TRUE, &&H_LOAD_FALSE, &&H_LOAD_CONST, &&H_LOAD_INT, &&H_LOAD_GLOBAL, &&H_STORE_GLOBAL, &&H_LOAD_GLOBAL_CACHED,              \
      &&H_STORE_GLOBAL_CACHED, &&H_MOVE, &&H_GET_UPVALUE, &&H_SET_UPVALUE, &&H_CLOSE_UPVALUE, &&H_OP_ADD, &&H_OP_ADD_II, &&H_OP_ADD_FF,              \
      &&H_OP_ADD_RI, &&H_OP_SUB, &&H_OP_SUB_II, &&H_OP_SUB_FF, &&H_OP_SUB_RI, &&H_OP_MUL, &&H_OP_MUL_II, &&H_OP_MUL_FF, &&H_OP_MUL_RI, &&H_OP_DIV,   \
      &&H_OP_DIV_II, &&H_OP_DIV_FF, &&H_OP_MOD, &&H_OP_MOD_II, &&H_OP_MOD_FF, &&H_OP_POW, &&H_OP_NEG, &&H_OP_NEG_I, &&H_OP_NEG_F, &&H_OP_BITAND,     \
      &&H_OP_BITAND_I, &&H_OP_BITOR, &&H_OP_BITOR_I, &&H_OP_BITXOR, &&H_OP_BITXOR_I, &&H_OP_BITNOT, &&H_OP_LSHIFT, &&H_OP_RSHIFT, &&H_OP_EQ,         \
      &&H_OP_EQ_II, &&H_OP_EQ_FF, &&H_OP_EQ_SS, &&H_OP_EQ_RI, &&H_OP_NEQ, &&H_OP_NEQ_II, &&H_OP_NEQ_FF, &&H_OP_NEQ_SS, &&H_OP_LT, &&H_OP_LT_II,      \
      &&H_OP_LT_FF, &&H_OP_LT_SS, &&H_OP_LT_RI, &&H_OP_LTE, &&H_OP_LTE_II, &&H_OP_LTE_FF, &&H_OP_LTE_SS, &&H_OP_LTE_RI, &&H_OP_NOT, &&H_CONCAT,      \
      &&H_LIST_NEW, &&H_LIST_APPEND, &&H_LIST_GET, &&H_LIST_SET, &&H_LIST_LEN, &&H_JUMP, &&H_JUMP_IF_TRUE, &&H_JUMP_IF_FALSE, &&H_LOOP,              \
      &&H_FOR_PREP, &&H_FOR_STEP, &&H_CLOSURE, &&H_CALL, &&H_CALL_TAIL, &&H_RETURN, &&H_RETURN_NIL, &&H_RETURN1, &&H_IC_CALL, &&H_NOP, &&H_HALT,
...
#define CLOSE_UPVALUES_IF_NEEDED()                                                                                                                   \
  do {                                                                                                                                               \
    if (__builtin_expect(has_open_upvalues > 0, 0)) {                                                                                                \
...
Value VM::execute() {
  ...
  CASE(OP_ADD_RI) {
    ...
    RA() = __builtin_expect(IS_INTEGER(lhs), 1) ? MAKE_INTEGER(AS_INTEGER(lhs) + imm) : MAKE_REAL(AS_DOUBLE(lhs) + imm);
    DISPATCH();
  }
  CASE(OP_SUB) {
    uint8_t a = instr_A(instr), b = instr_B(instr), c = instr_C(instr);
  ...
  CASE(OP_NEG) {
    ...
    if (__builtin_expect(!IS_INTEGER(operand), 0))
      runtimeError(ErrorCode::TYPE_ERROR_ARITH);
  ...
  CASE(OP_LSHIFT) { DISPATCH(); }
  CASE(OP_RSHIFT) { DISPATCH(); }
  CASE(OP_EQ) {
    uint8_t a = instr_A(instr), b = instr_B(instr), c = instr_C(instr);
    Value lhs = cur_base[b], rhs = cur_base[c];
    if (IS_INTEGER(lhs) & IS_INTEGER(rhs)) {
  ...
  CASE(OP_EQ_RI) {
    Value lhs = RB();
    int8_t imm = static_cast<int8_t>(instr_C(instr) - 128);
    if (__builtin_expect(IS_INTEGER(lhs), 1))
      RA() = MAKE_BOOL(AS_INTEGER(lhs) == static_cast<int64_t>(imm));
    else {
      if (__builtin_expect(!IS_DOUBLE(lhs), 0))
        runtimeError(ErrorCode::TYPE_ERROR_ARITH);
  ...
  CASE(FOR_PREP) {
    ...
    if (__builtin_expect(!IS_INTEGER(init_v) || !IS_INTEGER(limit_v) || !IS_INTEGER(step_v), 0))
      runtimeError(ErrorCode::TYPE_ERROR_ARITH);
  ...
  CASE(CALL) {
    ...
    if (IS_CLOSURE(callee)) [[likely]] {
      ...
      if (__builtin_expect(argc != fchk->arity, 0)) {
      ...
      if (__builtin_expect(FramesTop_ >= MAX_FRAMES, 0))
...
void VM::callValue(Value callee, int argc, int base, bool tail) {
  if (IS_CLOSURE(callee)) {
    ...
    if (__builtin_expect(argc != arity, 0)) {
  ...
  if (IS_NATIVE(callee)) {
    ...
    if (nat->arity >= 0 && argc != nat->arity) [[unlikely]] {
      diagnostic::emit("native '" + std::string(nat->name ? nat->name->str.data() : "?") + "' expected " + std::to_string(nat->arity) +
...
void VM::registerNative(StringRef const &name, NativeFn fn, int arity) {
  ...
  Value val = MAKE_OBJECT(MAKE_OBJ_NATIVE(fn, name_obj, arity));
  auto it = GlobalIndex_.find(name);
  if (it != GlobalIndex_.end()) {
