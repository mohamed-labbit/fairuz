...
ast::ExprPtr Parser::parseExpression()
{
  ...
  if (check(lex::tok::TokenType::OP_STAR))
  {
    advance();
    ast::ExprPtr expr = parseExpression();
    return ast::ExprPtr(std::move(expr));
  }
...
ast::ExprPtr Parser::parseAssignmentExpr()
{
  ast::ExprPtr left = parseConditionalExpr();
      ...
      check(lex::tok::TokenType::PERCENT_ASSIGN)*/)
  {
    ...
    advance();
    ast::ExprPtr right = parseAssignmentExpr();  // Right associative
    return std::make_unique<ast::AssignmentExpr>(std::move(left), std::move(right));
  }
...
ast::ExprPtr Parser::parseLogicalExpr() { return parseLogicalExprPrecedence(0); }

ast::ExprPtr Parser::parseLogicalExprPrecedence(int min_precedence)
{
  ...
  */
  ...
  ast::ExprPtr left = parseComparisonExpr();
  ...
  while (true)
  {
    ...
    ast::ExprPtr right = parseLogicalExprPrecedence(precedence + 1);
...
ast::ExprPtr Parser::parseComparisonExpr()
{
         ...
         || check(lex::tok::TokenType::OP_GTE) /*|| check(lex::tok::TokenType::OP_IN) || check(lex::tok::TokenType::IS)*/)
  {
    string_type op = lex_.current().lexeme();
    advance();
    ast::ExprPtr right = parseBinaryExpr();
    left               = std::make_unique<ast::BinaryExpr>(std::move(left), std::move(right), op);
  }
...
ast::ExprPtr Parser::parseBinaryExpr() { return parseBinaryExprPrecedence(0); }

ast::ExprPtr Parser::parseBinaryExprPrecedence(int min_precedence)
{
  ast::ExprPtr left = parseUnaryExpr();
...
ast::ExprPtr Parser::parseUnaryExpr()
{
  if (check(lex::tok::TokenType::OP_PLUS) || check(lex::tok::TokenType::OP_MINUS) /*|| check(lex::tok::TokenType::TILDE)*/)
  {
    string_type op = lex_.current().lexeme();
    advance();
    ast::ExprPtr expr = parseUnaryExpr();
    return std::make_unique<ast::UnaryExpr>(std::move(expr), op);
  }
...
ast::ExprPtr Parser::parsePrimaryExpr()
{
  ...
  if (check(lex::tok::TokenType::NUMBER))
  {
    ...
    advance();
    return std::make_unique<ast::LiteralExpr>(ast::LiteralExpr::Type::NUMBER, value);
  }
  ...
  */
  ...
  {
    advance();
    return std::make_unique<ast::LiteralExpr>(ast::LiteralExpr::Type::NONE, u"");
  }
  ...
  if (check(lex::tok::TokenType::IDENTIFIER))
  {
    ...
    advance();
    return std::make_unique<ast::NameExpr>(name);
  }
