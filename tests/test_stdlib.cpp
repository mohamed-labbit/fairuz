...
static Value makeStr(char const *s) { return MAKE_OBJECT(MAKE_OBJ_STRING(StringRef(s))); }

static Value makeList(std::initializer_list<Value> elems = {}) {
  Value v = MAKE_OBJECT(MAKE_OBJ_LIST());
...
TEST(NativeSubstr, BasicSubstr) {
  Value r = call(nativeSubstr, {makeStr("hello"), MAKE_INTEGER(1), MAKE_INTEGER(3)});
  if (!IS_NIL(r)) {
    ASSERT_TRUE(IS_STRING(r));
...
TEST(NativeSubstr, FromStart) {
  Value r = call(nativeSubstr, {makeStr("hello"), MAKE_INTEGER(0), MAKE_INTEGER(2)});
  if (!IS_NIL(r)) {
    ASSERT_TRUE(IS_STRING(r));
