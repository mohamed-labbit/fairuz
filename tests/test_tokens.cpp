...
TEST(LexerTest, RecognizesPlus)
{
    ...
    tok::Token const* expected = MAKE_TOKEN(tok::TokenType::OP_PLUS, "+", 1, 1);
...
TEST(LexerTest, RecognizesInteger)
{
    ...
    lex::Lexer lexer(&file_manager);
    auto tokens = lexer.tokenize();
    tok::Token const* expected = MAKE_TOKEN(tok::TokenType::INTEGER, "123", 1, 1);
    EXPECT_EQ(tokens.size(), 3);
    EXPECT_EQ(tokens[0]->type(), tok::TokenType::BEGINMARKER);
    EXPECT_EQ(*tokens[1], *expected);
    EXPECT_EQ(tokens[2]->type(), tok::TokenType::ENDMARKER);
...
TEST(LexerTest, RecognizesFloat)
{
    ...
    lex::Lexer lexer(&file_manager);
    auto tokens = lexer.tokenize();
    tok::Token const* expected = MAKE_TOKEN(tok::TokenType::DECIMAL, "123.456", 1, 1);
    EXPECT_EQ(tokens.size(), 3);
    EXPECT_EQ(tokens[0]->type(), tok::TokenType::BEGINMARKER);
    EXPECT_EQ(*tokens[1], *expected);
    EXPECT_EQ(tokens[2]->type(), tok::TokenType::ENDMARKER);
...
TEST(LexerTest, RecognizesIdentifier)
{
    ...
    lex::Lexer lexer(&file_manager);
    auto tokens = lexer.tokenize();
    tok::Token const* expected = MAKE_TOKEN(tok::TokenType::IDENTIFIER, "مرحبا", 1, 1);
    EXPECT_EQ(tokens.size(), 3);
    EXPECT_EQ(tokens[0]->type(), tok::TokenType::BEGINMARKER);
    EXPECT_EQ(*tokens[1], *expected);
    EXPECT_EQ(tokens[2]->type(), tok::TokenType::ENDMARKER);
...
TEST(LexerTest, RecognizesStringLiteral)
{
    ...
    lex::Lexer lexer(&file_manager);
    auto tokens = lexer.tokenize();
    tok::Token const* expected = MAKE_TOKEN(tok::TokenType::STRING, "العالم", 1, 1);
    EXPECT_EQ(tokens.size(), 3);
    EXPECT_EQ(tokens[0]->type(), tok::TokenType::BEGINMARKER);
    EXPECT_EQ(*tokens[1], *expected);
    EXPECT_EQ(tokens[2]->type(), tok::TokenType::ENDMARKER);
...
TEST(LexerTest, RecognizesExpression00)
{
    ...
    std::vector<tok::Token const*> expected = {
        MAKE_TOKEN(tok::TokenType::BEGINMARKER, "", 1, 1), MAKE_TOKEN(tok::TokenType::IDENTIFIER, "س", 1, 1),
        MAKE_TOKEN(tok::TokenType::OP_EQ, "=", 1, 3), MAKE_TOKEN(tok::TokenType::INTEGER, "42", 1, 5),
        MAKE_TOKEN(tok::TokenType::OP_PLUS, "+", 1, 8), MAKE_TOKEN(tok::TokenType::IDENTIFIER, "ي", 1, 10),
        MAKE_TOKEN(tok::TokenType::ENDMARKER, "", 1, 10)
    };
...
TEST(LexerTest, RecognizesStmt00)
{
    ...
    auto tokens = lexer.tokenize();
    std::vector<tok::Token const*> expected = {
        MAKE_TOKEN(tok::TokenType::BEGINMARKER, "", 1, 1), MAKE_TOKEN(tok::TokenType::KW_IF, "اذا", 1, 1),
        MAKE_TOKEN(tok::TokenType::IDENTIFIER, "س", 1, 5), MAKE_TOKEN(tok::TokenType::OP_EQ, "=", 1, 7),
        MAKE_TOKEN(tok::TokenType::IDENTIFIER, "د", 1, 9), MAKE_TOKEN(tok::TokenType::COLON, ":", 1, 10),
        MAKE_TOKEN(tok::TokenType::ENDMARKER, "", 1, 10)
    };
...
TEST(LexerTest, RecognizesStmt01)
{
    ...
    auto tokens = lexer.tokenize();
    std::vector<tok::Token const*> expected = {
        MAKE_TOKEN(tok::TokenType::BEGINMARKER, "", 1, 1), MAKE_TOKEN(tok::TokenType::KW_WHILE, "طالما", 1, 1),
        MAKE_TOKEN(tok::TokenType::IDENTIFIER, "س", 1, 7), MAKE_TOKEN(tok::TokenType::OP_NEQ, "!=", 1, 9),
        MAKE_TOKEN(tok::TokenType::IDENTIFIER, "د", 1, 12), MAKE_TOKEN(tok::TokenType::COLON, ":", 1, 13),
        MAKE_TOKEN(tok::TokenType::ENDMARKER, "", 1, 13)
    };
    EXPECT_EQ(tokens.size(), expected.size());
    for (size_t i = 0; i < tokens.size(); ++i)
        EXPECT_EQ(*tokens[i], *expected[i]);
...
TEST(LexerTest, RecognizesStmt02)
{
    ...
    auto tokens = lexer.tokenize();
    std::vector<tok::Token const*> expected = {
        MAKE_TOKEN(tok::TokenType::BEGINMARKER, "", 1, 1), MAKE_TOKEN(tok::TokenType::KW_FOR, "بكل", 1, 1),
        MAKE_TOKEN(tok::TokenType::IDENTIFIER, "ل", 1, 5), MAKE_TOKEN(tok::TokenType::IDENTIFIER, "في", 1, 7),
        MAKE_TOKEN(tok::TokenType::IDENTIFIER, "ك", 1, 10), MAKE_TOKEN(tok::TokenType::COLON, ":", 1, 11),
        MAKE_TOKEN(tok::TokenType::ENDMARKER, "", 1, 11)
    };
    EXPECT_EQ(tokens.size(), expected.size());
    for (size_t i = 0; i < tokens.size(); ++i)
        EXPECT_EQ(*tokens[i], *expected[i]);
...
TEST(LexerTest, RecognizesStmt03)
{
    ...
    auto tokens = lexer.tokenize();
    std::vector<tok::Token const*> expected = {
        MAKE_TOKEN(tok::TokenType::BEGINMARKER, "", 1, 1),
        MAKE_TOKEN(tok::TokenType::IDENTIFIER, "ا", 1, 1),
        MAKE_TOKEN(tok::TokenType::OP_ASSIGN, ":=", 1, 3),
        MAKE_TOKEN(tok::TokenType::KW_FALSE, "خطا", 1, 6),
        MAKE_TOKEN(tok::TokenType::ENDMARKER, "", 1, 8),
    };
    EXPECT_EQ(tokens.size(), expected.size());
    for (size_t i = 0; i < tokens.size(); ++i)
        EXPECT_EQ(*tokens[i], *expected[i]);
...
TEST(LexerTest, RecognizesStmt04)
{
    ...
    std::vector<tok::Token const*> expected = {
        MAKE_TOKEN(tok::TokenType::BEGINMARKER, "", 1, 1), MAKE_TOKEN(tok::TokenType::KW_IF, "اذا", 1, 1),
        MAKE_TOKEN(tok::TokenType::IDENTIFIER, "ا", 1, 5), MAKE_TOKEN(tok::TokenType::OP_EQ, "=", 1, 7),
        MAKE_TOKEN(tok::TokenType::INTEGER, "3", 1, 9), MAKE_TOKEN(tok::TokenType::COLON, ":", 1, 10),
        MAKE_TOKEN(tok::TokenType::NEWLINE, "\n", 1, 11), MAKE_TOKEN(tok::TokenType::INDENT, "", 0, 0),
        MAKE_TOKEN(tok::TokenType::KW_RETURN, "ارجع", 2, 5), MAKE_TOKEN(tok::TokenType::DEDENT, "", 0, 0),
        MAKE_TOKEN(tok::TokenType::ENDMARKER, "", 2, 8)
    };
