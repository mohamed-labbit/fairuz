...
TEST(LexerTest, RecognizesInteger)
{
  ...
  lex::Lexer              lexer(&file_manager);
  std::vector<const tok::Token*> tokens   = lexer.tokenize();
  const tok::Token*              expected = lexer.make_token(tok::TokenType::NUMBER, u"123", 1, 1);
  EXPECT_EQ(tokens.size(), 3);
  EXPECT_EQ(tokens[0]->type(), tok::TokenType::BEGINMARKER);
  EXPECT_EQ(*tokens[1], *expected);
  EXPECT_EQ(tokens[2]->type(), tok::TokenType::ENDMARKER);
...
TEST(LexerTest, RecognizesFloat)
{
  ...
  lex::Lexer              lexer(&file_manager);
  std::vector<const tok::Token*> tokens   = lexer.tokenize();
  const tok::Token*              expected = lexer.make_token(tok::TokenType::FLOAT, u"123.456", 1, 1);
  EXPECT_EQ(tokens.size(), 3);
  EXPECT_EQ(tokens[0]->type(), tok::TokenType::BEGINMARKER);
  EXPECT_EQ(*tokens[1], *expected);
  EXPECT_EQ(tokens[2]->type(), tok::TokenType::ENDMARKER);
...
TEST(LexerTest, RecognizesIdentifier)
{
  ...
  lex::Lexer              lexer(&file_manager);
  std::vector<const tok::Token*> tokens   = lexer.tokenize();
  const tok::Token*              expected = lexer.make_token(tok::TokenType::IDENTIFIER, u"مرحبا", 1, 1);
  EXPECT_EQ(tokens.size(), 3);
  EXPECT_EQ(tokens[0]->type(), tok::TokenType::BEGINMARKER);
  EXPECT_EQ(*tokens[1], *expected);
  EXPECT_EQ(tokens[2]->type(), tok::TokenType::ENDMARKER);
...
TEST(LexerTest, RecognizesStringLiteral)
{
  ...
  lex::Lexer              lexer(&file_manager);
  std::vector<const tok::Token*> tokens   = lexer.tokenize();
  const tok::Token*              expected = lexer.make_token(tok::TokenType::STRING, u"العالم", 1, 1);
  EXPECT_EQ(tokens.size(), 3);
  EXPECT_EQ(tokens[0]->type(), tok::TokenType::BEGINMARKER);
  EXPECT_EQ(*tokens[1], *expected);
  EXPECT_EQ(tokens[2]->type(), tok::TokenType::ENDMARKER);
...
TEST(LexerTest, RecognizesExpression00)
{
  ...
  lex::Lexer              lexer(&file_manager);
  std::vector<const tok::Token*> tokens   = lexer.tokenize();
  std::vector<const tok::Token*> expected = {
...
TEST(LexerTest, RecognizesStmt00)
{
  ...
  lex::Lexer              lexer(&file_manager);
  std::vector<const tok::Token*> tokens   = lexer.tokenize();
  std::vector<const tok::Token*> expected = {lexer.make_token(tok::TokenType::BEGINMARKER, u"", 1, 1), lexer.make_token(tok::TokenType::KW_IF, u"اذا", 1, 1),
