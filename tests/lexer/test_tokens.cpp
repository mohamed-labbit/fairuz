...
inline void PrintTo(const lex::tok::Token& tok, std::ostream* os)
{
  *os << "lex::tok::Token(\"" << utf8::utf16to8(tok.lexeme()) << "\", type=" << static_cast<int>(tok.type()) << ", line=" << tok.line()
      << ", col=" << tok.column() << ")";
}
...
TEST(LexerTest, RecognizesPlus)
{
  ...
  auto        expected = lexer.make_token(lex::tok::TokenType::OP_PLUS, u"+", 1, 1);
  EXPECT_EQ(tokens.size(), 3);
  EXPECT_EQ(tokens[0].type(), lex::tok::TokenType::BEGINMARKER);
  EXPECT_EQ(tokens[1], expected);
  EXPECT_EQ(tokens[2].type(), lex::tok::TokenType::ENDMARKER);
}
...
TEST(LexerTest, RecognizesInteger)
{
  ...
  lex::Lexer  lexer(&file_manager);
  auto        tokens   = lexer.tokenize();
  auto        expected = lexer.make_token(lex::tok::TokenType::NUMBER, u"123", 1, 1);
  EXPECT_EQ(tokens.size(), 3);
  EXPECT_EQ(tokens[0].type(), lex::tok::TokenType::BEGINMARKER);
  EXPECT_EQ(tokens[1], expected);
  EXPECT_EQ(tokens[2].type(), lex::tok::TokenType::ENDMARKER);
...
TEST(LexerTest, RecognizesFloat)
{
  ...
  lex::Lexer  lexer(&file_manager);
  auto        tokens   = lexer.tokenize();
  auto        expected = lexer.make_token(lex::tok::TokenType::FLOAT, u"123.456", 1, 1);
  EXPECT_EQ(tokens.size(), 3);
  EXPECT_EQ(tokens[0].type(), lex::tok::TokenType::BEGINMARKER);
  EXPECT_EQ(tokens[1], expected);
  EXPECT_EQ(tokens[2].type(), lex::tok::TokenType::ENDMARKER);
...
TEST(LexerTest, RecognizesIdentifier)
{
  ...
  lex::Lexer  lexer(&file_manager);
  auto        tokens   = lexer.tokenize();
  auto        expected = lexer.make_token(lex::tok::TokenType::IDENTIFIER, u"مرحبا", 1, 1);
  EXPECT_EQ(tokens.size(), 3);
  EXPECT_EQ(tokens[0].type(), lex::tok::TokenType::BEGINMARKER);
  EXPECT_EQ(tokens[1], expected);
  EXPECT_EQ(tokens[2].type(), lex::tok::TokenType::ENDMARKER);
...
TEST(LexerTest, RecognizesKeyword)
{
  ...
  EXPECT_EQ(tokens.size(), 3);
  EXPECT_EQ(tokens[0].type(), lex::tok::TokenType::BEGINMARKER);
  EXPECT_EQ(tokens[1].type(), lex::tok::TokenType::KW_IF);
  EXPECT_EQ(tokens[2].type(), lex::tok::TokenType::ENDMARKER);
}
...
TEST(LexerTest, RecognizesNoneKeyword)
{
  ...
  lex::Lexer  lexer(&file_manager);
  auto        tokens  = lexer.tokenize();
  auto        none_it = std::find_if(tokens.begin(), tokens.end(), [](const auto& tok) { return tok.type() == lex::tok::TokenType::KW_NONE; });
...
TEST(LexerTest, RecognizesBooleanKeywords)
{
  ...
  auto        tokens = lexer.tokenize();
  auto        has_true =
    std::any_of(tokens.begin(), tokens.end(), [](const auto& tok) { return tok.type() == lex::tok::TokenType::KW_TRUE && tok.lexeme() == u"صحيح"; });
  auto has_false =
    std::any_of(tokens.begin(), tokens.end(), [](const auto& tok) { return tok.type() == lex::tok::TokenType::KW_FALSE && tok.lexeme() == u"خطا"; });
...
TEST(LexerTest, RecognizesStringLiteral)
{
  ...
  lex::Lexer  lexer(&file_manager);
  auto        tokens   = lexer.tokenize();
  auto        expected = lexer.make_token(lex::tok::TokenType::STRING, u"العالم", 1, 1);
  EXPECT_EQ(tokens.size(), 3);
  EXPECT_EQ(tokens[0].type(), lex::tok::TokenType::BEGINMARKER);
  EXPECT_EQ(tokens[1], expected);
  EXPECT_EQ(tokens[2].type(), lex::tok::TokenType::ENDMARKER);
...
TEST(LexerTest, RecognizesExpression00)
{
  ...
  lex::Lexer                   lexer(&file_manager);
  auto                         tokens   = lexer.tokenize();
  std::vector<lex::tok::Token> expected = {
    lexer.make_token(lex::tok::TokenType::BEGINMARKER, u"", 1, 1), lexer.make_token(lex::tok::TokenType::IDENTIFIER, u"س", 1, 1),
    lexer.make_token(lex::tok::TokenType::OP_EQ, u"=", 1, 3),      lexer.make_token(lex::tok::TokenType::NUMBER, u"42", 1, 5),
    lexer.make_token(lex::tok::TokenType::OP_PLUS, u"+", 1, 8),    lexer.make_token(lex::tok::TokenType::IDENTIFIER, u"ي", 1, 10),
    lexer.make_token(lex::tok::TokenType::ENDMARKER, u"", 1, 10)};
...
TEST(LexerTest, RecognizesStmt00)
{
  ...
  std::vector<lex::tok::Token> expected = {
    ...
    lexer.make_token(lex::tok::TokenType::IDENTIFIER, u"س", 1, 5), lexer.make_token(lex::tok::TokenType::OP_EQ, u"=", 1, 7),
    lexer.make_token(lex::tok::TokenType::IDENTIFIER, u"د", 1, 9), lexer.make_token(lex::tok::TokenType::COLON, u":", 1, 10),
    lexer.make_token(lex::tok::TokenType::ENDMARKER, u"", 1, 10)};
...
TEST(LexerTest, RecognizesStmt01)
{
  ...
  auto                         tokens   = lexer.tokenize();
  std::vector<lex::tok::Token> expected = {
    lexer.make_token(lex::tok::TokenType::BEGINMARKER, u"", 1, 1),  lexer.make_token(lex::tok::TokenType::KW_WHILE, u"طالما", 1, 1),
    lexer.make_token(lex::tok::TokenType::IDENTIFIER, u"س", 1, 7),  lexer.make_token(lex::tok::TokenType::OP_NEQ, u"!=", 1, 9),
    lexer.make_token(lex::tok::TokenType::IDENTIFIER, u"د", 1, 12), lexer.make_token(lex::tok::TokenType::COLON, u":", 1, 13),
    lexer.make_token(lex::tok::TokenType::ENDMARKER, u"", 1, 13)};
  EXPECT_EQ(tokens.size(), expected.size());
  EXPECT_EQ(tokens, expected);
}
...
TEST(LexerTest, RecognizesStmt02)
{
  ...
  auto                         tokens   = lexer.tokenize();
  std::vector<lex::tok::Token> expected = {
    lexer.make_token(lex::tok::TokenType::BEGINMARKER, u"", 1, 1),  lexer.make_token(lex::tok::TokenType::KW_FOR, u"بكل", 1, 1),
    lexer.make_token(lex::tok::TokenType::IDENTIFIER, u"ل", 1, 5),  lexer.make_token(lex::tok::TokenType::IDENTIFIER, u"في", 1, 7),
    lexer.make_token(lex::tok::TokenType::IDENTIFIER, u"ك", 1, 10), lexer.make_token(lex::tok::TokenType::COLON, u":", 1, 11),
    lexer.make_token(lex::tok::TokenType::ENDMARKER, u"", 1, 11)};
  EXPECT_EQ(tokens.size(), expected.size());
  EXPECT_EQ(tokens, expected);
}
...
TEST(LexerTest, RecognizesStmt03)
{
  ...
  auto                         tokens   = lexer.tokenize();
  std::vector<lex::tok::Token> expected = {
    lexer.make_token(lex::tok::TokenType::BEGINMARKER, u"", 1, 1), lexer.make_token(lex::tok::TokenType::IDENTIFIER, u"ا", 1, 1),
    lexer.make_token(lex::tok::TokenType::OP_ASSIGN, u":=", 1, 3), lexer.make_token(lex::tok::TokenType::KW_FALSE, u"خطا", 1, 6),
    lexer.make_token(lex::tok::TokenType::ENDMARKER, u"", 1, 8),
  };
  EXPECT_EQ(tokens.size(), expected.size());
  EXPECT_EQ(tokens, expected);
}
