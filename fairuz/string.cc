...
StringBase::StringBase(char const* s, size_t n)
{
    ...
    if (!m_is_heap) {
        ...
        if (m_storage.heap.ptr == nullptr)
            diagnostic::panic(diagnostic::errc::general::Code::ALLOC_FAILED, "allocateArray<char>(size=" + std::to_string(m_storage.heap.cap) + ") failed!");
...
char Fa_StringRef::at(size_t const i) const
{
    if (UNLIKELY(i >= m_length))
        diagnostic::emit(diagnostic::errc::general::Code::INTERNAL_ERROR, "Fa_StringRef::at: index out of bounds", diagnostic::Severity::FATAL);
...
Fa_StringRef Fa_StringRef::substr_copy(size_t start, size_t end) const
{
    if (m_length == 0)
        return { };
    if (start > m_length)
        diagnostic::emit(diagnostic::errc::general::Code::INTERNAL_ERROR, "Fa_StringRef::substrCopy: start index out of range", diagnostic::Severity::FATAL);
...
{
    if (empty())
        diagnostic::emit(diagnostic::errc::general::Code::INTERNAL_ERROR, "Fa_StringRef::toDouble: empty string", diagnostic::Severity::FATAL);

    f64 result { };
    auto [end_ptr, ec] = std::from_chars(data(), data() + m_length, result);

    if (ec == std::errc::invalid_argument)
        diagnostic::emit(diagnostic::errc::general::Code::INTERNAL_ERROR, "Fa_StringRef::toDouble: invalid number format", diagnostic::Severity::FATAL);
    if (ec == std::errc::result_out_of_range)
        diagnostic::emit(diagnostic::errc::general::Code::INTERNAL_ERROR, "Fa_StringRef::toDouble: number out of range", diagnostic::Severity::FATAL);
...
Fa_StringRef Fa_StringRef::from_utf16(char16_t const* src)
{
    ...
    if (validation.error != simdutf::error_code::SUCCESS)
        diagnostic::emit(diagnostic::errc::general::Code::INTERNAL_ERROR, "Invalid UTF-16 at code unit " + std::to_string(validation.count), diagnostic::Severity::FATAL);
