...
std::optional<Fa_Value> try_fold_binary(AST::Fa_BinaryExpr const* e)
{
    ...
    auto make_literal_from_val = [](Fa_Value const v, Fa_SourceLocation loc) {
        if (Fa_IS_DOUBLE(v))
            return AST::Fa_makeLiteralFloat(Fa_AS_DOUBLE(v), loc);
        if (Fa_IS_INTEGER(v))
            return AST::Fa_makeLiteralInt(Fa_AS_INTEGER(v), loc);
        if (Fa_IS_BOOL(v))
            return AST::Fa_makeLiteralBool(Fa_AS_BOOL(v), loc);

        return AST::Fa_makeLiteralNil(loc);
    };
